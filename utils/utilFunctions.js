...
const { servername } = require("../config.js");
...
const cleanFilledValues = (role) => (role.includes("~~Filled ${servername} Spot~~") ? role.slice(0, -1) : role);

const filterSpots = (spots, interactionUserId, reason) => {
    if (reason === "cancelled") {
        return spots.filter((member) => member !== interactionUserId && !member.includes("~~Filled ${servername} Spot"));
    } else {
        return spots.filter((member) => !member.includes("~~Filled ${servername} Spot"));
    }
};
...
function generateListedAsString(dungeon) {
    ...
    return `${servername} ${dungeonAcronym} ${randomLetterPair}`;
}
